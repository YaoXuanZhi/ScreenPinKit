...
class DetResizeForTest(object):
    ...
    def image_padding(self, im, value=0):
        h, w, c = im.shape
        im_pad = (
            np.zeros((max(model_shape_w, h), max(model_shape_w, w), c), np.uint8)
            + value
        )
        im_pad[:h, :w, :] = im
        return im_pad
    ...
    def resize_image_type0(self, img):
        ...
        resize_h = max(
            int(round(resize_h / model_shape_w) * model_shape_w), model_shape_w
        )
        resize_w = max(
            int(round(resize_w / model_shape_w) * model_shape_w), model_shape_w
        )
...
class det_rec_functions(object):
    def __init__(self, image, use_dnn=False, version=3):
        ...
        self.img = image.copy()
        self.det_file = os.path.join(
            module_dir, "modelv{}/det_model.onnx".format(version)
        )
        self.small_rec_file = os.path.join(
            module_dir, "modelv{}/rec_model.onnx".format(version)
        )
        ...
        self.postprocess_op = process_pred(
            os.path.join(module_dir, "ppocr_keys_v1.txt"), "ch", True
        )
